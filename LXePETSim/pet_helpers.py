...
def add_LXe_digitizer_v1(sim, pet, output):
    """
    add a  PET digitizer.
    ...
    tb.blur_fwhm = 110.0 * ps

    # EnergyWindows
    ew = sim.add_actor("DigitizerEnergyWindowsActor", "Singles5")
    ew.output_filename = output
    ew.input_digi_collection = tb.name
    ew.channels = [{"name": ew.name, "min": 489.54 * keV, "max": 532.462 * keV}]

    #PositionBlur
    # pb = sim.add_actor("DigitizerSpatialBlurringActor", "Singles6")
    # pb.output_filename = output
    # pb.input_digi_collection = "Singles5"
    # pb.blur_attribute = "PostPosition"
    # pb.blur_fwhm = 0.56 * mm
