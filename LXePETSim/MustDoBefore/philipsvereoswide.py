...
def add_pet(sim, name="pet", create_housing=True, create_mat=True, debug=False, shift=0):
    """
    ...
    # RING
    ring = sim.add_volume("Box", f"{name}_ring")
    ring.mother = pet.name
    ring.size = [1000 * mm, 1000 * mm, 164 * mm]
    # ring.rmax = 500 * mm
    # ring.rmin = 354 * mm
    # ring.dz = 770 * mm / 2.0
    ...
    ring.color = yellow
    # 1 ring
    ring.translation = get_grid_repetition([1, 1, 1], [0 * mm, 0 * mm, 189 * mm])
    # 3 rings
    # ring.translation = get_grid_repetition([1, 1, 3], [0 * mm, 0 * mm, 189 * mm])

    # Module (each module has 4x5 stacks)
    module = sim.add_volume("Box", f"{name}_module")
    module.mother = ring.name
    module.size = [(shift+19) * mm, 131.4 * mm, 164 * mm]
    module.material = "ABS"
    module.color = blue
    translations_ring, rotations_ring = get_circular_repetition(
        18, [(391.5+shift2) * mm, 0, 0], start_angle_deg=190, axis=[0, 0, 1]
    )
    module.translation = translations_ring
    # module.translation = get_grid_repetition([10, 10, 10], [30 * mm, 30 * mm, 30 * mm])
    ...
    # 3 rings 
    #endshielding1.translation = [0, 0, 284.5 * mm]
    endshielding1.rmax = (shift+410) * mm
    ...
    # 3 rings
    #endshielding2.translation = [0, 0, -284.5 * mm]
    endshielding2.rmax = (shift+410) * mm
